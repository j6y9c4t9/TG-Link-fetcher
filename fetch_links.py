...
def is_subscription_alive(link, ssl_context):
    """
    终极解密与探测：兼容 Base64、明文 YAML，自动清洗脏字符，智能测活
    """
    try:
        # 1. 伪装成现代 Mihomo 客户端请求原始订阅
        ...
        # 如果不是标准 YAML 格式，采用第二套正则匹配通用的 域名:端口 格式
        if not servers:
            # 匹配形如 [域名/IP]:[端口] 的组合
        ...
        # 4. 彻底清洗提取出来的域名和端口（干掉末尾的逗号、引号、空格）
        valid_pairs = []
        for s, p in zip(servers, ports):
            s_clean = s.strip("'\" ,\r\n\t").split(',')[0].split('"')[0].split("'")[0]
            p_clean = int(p)
            
            # 剔除常见的机场内置通告/高防盾垃圾伪节点
...
def main():
    try:
        ...
        print("🎉 [完美收工] 真正鲜活、有节点的机场订阅已成功融入你的高级 config.yaml。")
                
    except Exception as e:
        print(f"❌ 运行崩溃: {e}")

if __name__ == '__main__':
    main()            print("   ❌ 该订阅内无可用的有效代理节点。")
            return False
